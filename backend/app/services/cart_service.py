...
from ..repositories.product_repository import ProducRespository
from ..schemas.cart import CartResponse, CartItemUpdate,CartItem,CartItemCreate
from fastapi import HTTPException,status


class CartService:
    def __init__(self,db:Session):
        self.product_repository = ProducRespository(db)
        ...
        def get_cart_details(self,cart_data: Dict[int,int]) -> CartResponse:
            ...
            for product_id,quantity in cart_data.items():
                if product_id in products_dict:
                    ...
                    cart_item = CartItem(product_id=product_id,name=product.name,proce=product.price,quantity=quantity,subtotal=subtotal,image_url=product.image_url)
